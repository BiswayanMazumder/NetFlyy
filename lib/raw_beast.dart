...
import 'package:netflix/raw_beast_links.dart';
import 'package:netflix/uri_links.dart';
import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:firebase_auth/firebase_auth.dart';
class Raw_Beast extends StatefulWidget {
  const Raw_Beast({Key? key}) : super(key: key);

  @override
  State<Raw_Beast> createState() => _Raw_BeastState();
}

class _Raw_BeastState extends State<Raw_Beast> {
  @override
  ...
  void _checkIfStarred() async {
    ...
    if (user != null) {
      ...
      final movieRef = _firestore.collection('starred').doc(userId).collection('starredMovies').doc('Raw:Beast');
  ...
  Future<void> _toggleStar() async {
    ...
    if (user != null) {
      final userId = user.uid;
      final starredMoviesRef = _firestore.collection('starred').doc(userId).collection('starredMovies').doc('Raw:Beast');
  ...
  void saveRatingToFirestore(bool isLiked) {
    _firestore.collection('ratings_raw_beast').add({
      'isLiked': isLiked,
      'timestamp': FieldValue.serverTimestamp(),
    });
  }
            ...
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  image: DecorationImage(image: NetworkImage('https://emkldzxxityxmjkxiggw.supabase.co/storage/v1/object/public/Netfly%20Storage/raw%20beast%20-%20Made%20with%20Clipchamp.gif?t=2023-11-30T13%3A36%3A49.684Z'),fit: BoxFit.fitHeight,
                      opacity: 60.000),
              ...
              Positioned(child: SizedBox(
                width: 250.0,
                height: 800.0,
                child: Image.network('https://emkldzxxityxmjkxiggw.supabase.co/storage/v1/object/public/Netfly%20Storage/Raw(beast)%20logo.png'),
              )),
            ],
          ),
          Column(
              ...
              Container(
                ...
                child: Column(
                  children: [
                    ...
                    Padding(padding: EdgeInsets.all(10)),
                    Text(
                      '"Raw(Beast)" is an upcoming Indian espionage thriller film directed by Shantanu Bagchi. The narrative is set '
                          ...
                          'operation.The title, "Raw(Beast)," suggests that the mission is of significant importance, with potential national and strategic '
                          ...
                          ' promises to offer not only entertainment but also a glimpse into the sacrifices and heroism of the unsung heroes who played a crucial role in safeguarding the nation. The historical context of the 1971 war adds depth to the narrative, making "Raw(Beast)" an intriguing and patriotic tale of espionage and bravery.',
            ...
            children: [
              ...
              Align(
                  ...
                  child: StreamBuilder<QuerySnapshot>(
                      stream: _firestore.collection('ratings_raw_beast').snapshots(),
