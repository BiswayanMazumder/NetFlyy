...
class Mission_majnu extends StatefulWidget {
  const Mission_majnu({Key? key}) : super(key: key);

  @override
  State<Mission_majnu> createState() => _Mission_majnuState();
}

class _Mission_majnuState extends State<Mission_majnu> {
  @override
  ...
  void _checkIfStarred() async {
    ...
    if (user != null) {
      ...
      final movieRef = _firestore.collection('starred').doc(userId).collection('starredMovies').doc('Mission Majnu');
  ...
  Future<void> _toggleStar() async {
    ...
    if (user != null) {
      final userId = user.uid;
      final starredMoviesRef = _firestore.collection('starred').doc(userId).collection('starredMovies').doc('Mission Majnu');
  ...
  void saveRatingToFirestore(bool isLiked) {
    _firestore.collection('ratings_mission_majnu').add({
      'isLiked': isLiked,
      'timestamp': FieldValue.serverTimestamp(),
    });
  }
            ...
            children: [
              ...
              Align(
                  ...
                  child: StreamBuilder<QuerySnapshot>(
                      stream: _firestore.collection('ratings_mission_majnu').snapshots(),
